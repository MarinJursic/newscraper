...
export default function ExplorePage() {
    ...
    const handlePageChange = (page: number) => {
        ...
        const mainContainer = document.querySelector('main.h-screen.overflow-y-auto');
        if (mainContainer) {
                    ...
                    <div className="flex items-center justify-between mb-6">
                        <p className="text-sm text-slate-500">
                            {loading ? (
                                ...
                                <>
                                    Showing <span className="font-semibold text-slate-700">{paginatedArticles.length}</span> of{" "}
                                    <span className="font-semibold text-slate-700">{articles.length}</span> articles
                                    {searchQuery && (
                                        <span className="ml-1">
                                            for "<span className="text-violet-600">{searchQuery}</span>"
                                        </span>
                                    )}
                                </>
                            )}
                    ...
                    {loading ? (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {paginatedArticles.map((article, idx) => (
                                <ArticleCard key={article.id} article={article} idx={idx} animate={false} />
                            ))}
                        </div>
                    )}
